...
目前先固定：
- **8H / 2H / 30m** 為主流程
- **5m 只做 confirmed 斷鍊標註**
- 目標只吃 **2H 的 `+2`**
- 30m 先用 **斷鍊 3 次出場**（保守可 2 次）

目前先不要做：
- 動態切換 4H / 1H / 5m profile
- 5m 主進出場版本
- 8H 世界內高頻多 sortie 版本
- 止損自動上移到 5m break point 的進階版
...
## 10. Mark 的操作偏好（UI/IDE 友善是硬需求）

- Mark 偏好 **PyCharm 按 Play** 的工作流：直接改原碼裡的 CONFIG 變數再跑，不想記 CLI 參數與順序。
- 因此每個 milestone 都應提供一支「可直接 Run」的入口腳本（例如 `run_*.py`），避免把必要參數藏在指令列。
- 專案結構要乾淨：新版本主程式與舊 runner 嚴格分離，舊檔統一丟 `archive/`，避免選錯檔。

## 11. 與 Claude Code 的協作方式
