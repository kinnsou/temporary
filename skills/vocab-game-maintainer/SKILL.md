...
- **Boss timer bar**: boss quiz screens show a top-edge countdown bar using `quest.bossClearSec`. Default boss per-question limits drop by 5s per boss: stage 5=50s, 10=45s, 15=40s, 20=35s, 25+=30s floor. The number counts down per question and the existing `clearWins` gate only counts correct answers inside the limit.
- **Leaderboard eligibility**: both 等級排行 and 連續天數 hide visitor-grade rows only when all three are true: `streakCur === 1`, today's quiz count is 0, and `level === 1`. Older/real players remain listed even if they have not answered today.
...
- **Sentence-order cap per quiz**: 3 (`MAX_ORDER`). Sentence questions use shuffled draggable/tappable word chips, a `送出答案` button, and speak the complete sentence after submission.
- **Pet hint budget**: 3 per quiz (`HINT_BUDGET = 3`). zh2en eliminates 2 wrong options, cloze speaks target via TTS, tense reveals `tenseHintText(form, base)` together with the base form (e.g. "（原形：wear）"), and sentence-order arranges the first 3 words. Hint state resets per question, budget persists across questions. **Per-question lock**: `hintUsedThisQ` flag — the pet can be tapped only once per question; subsequent taps do nothing (don't re-debit). The pet image acts as the hint button; the pill reads "{petName}提示 N/3" (e.g. "奶油貓提示 3/3"). Tense card **must not** show 原形 outside the hint — it leaks the answer when one of the 4 options equals the base form.
- **EXP per correct answer**: `calcExpGain(combo)` = 10, +5 at combo≥3, +10 at combo≥5, +20 at combo≥10.
- **Level XP curve** (`levelXpRequirement(level)`): 200, 260, 352, 492, 714, **1072**, then **caps linear at 1072** for every higher level. These are the original rounded requirements ×2. Multiplier sequence internally remains 1.30, 1.35, 1.40, 1.45, 1.50, then caps with `Math.min(LEVEL_XP_CAP/2, ...)` before doubling. Earlier exponential curve was explicitly rejected by Mark.
